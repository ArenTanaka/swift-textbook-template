...
本章では、PhotosPickerを利用して写真を選択し、Core Imageを使って画像へフィルターを適用する方法を学びました。
また、フィルターごとのサムネイルを表示して加工結果を確認する方法や、加工した画像をフォトライブラリへ保存する方法についても学びました。
Core Imageによる画像加工とPhotosフレームワークを組み合わせることで、写真の編集から保存までをSwiftUIアプリで実現できるようになりました。
