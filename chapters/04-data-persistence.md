...
入力されたタイトルと内容から新しい Memo データを作成し、modelContext.insert(memo) で SwiftData に保存している

**なぜこう書くのか：**

SwiftData に新しいデータを追加するには、modelContext を使って保存処理を行う必要があるため。

**もしこう書かなかったら：**

入力したメモが保存されず、一覧画面に表示できない。
...
保存されているメモ一覧を取り出して、memosとして使えるようにしている

**なぜこう書くのか：**

メモ帳アプリで保存済みのメモ一覧を画面に持ってくるための

**もしこう書かなかったら：**

保存したメモを画面に表示するために、
@QueryでSwiftDataからMemoデータを取得している。
---
...
この画面では @Binding で受け取っている。
保存処理は親View側の @AppStorage が担当している
...
@Bindingの受け取りがないからユーザーネームと、お気に入りの登録ができない??
...
1. **質問：**
   ...
   **得られた理解：**
   ...
   一時的な表記ではなくアプリを閉じても残るのは大きい

2. **質問：**
  
   AppStorageは宣言するだけで使えるものであって 特別にどこかで呼び出したりするものではないんですか??
